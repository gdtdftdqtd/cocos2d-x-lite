...
namespace RVO {
    ...
    bool RVOSimulator::removeAgent(size_t agentNo)
    {
        bool isRemove = false;
        std::vector<Agent *>::iterator it = agents_.begin();
        for (size_t i=0; it != agents_.end(); ++i,it++) {
            if (i == agentNo) {
                delete *it;
                *it = nullptr;
                agents_.erase(it);
        ...
        return isRemove && this->kdTree_->removeAgent(agentNo);
    }
