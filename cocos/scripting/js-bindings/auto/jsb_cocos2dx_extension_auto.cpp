...
bool js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough(JSContext *cx, uint32_t argc, JS::Value *vp)
{
    JS::CallArgs args = JS::CallArgsFromVp(argc, vp);
    bool ok = true; CC_UNUSED_PARAM(ok);
    JS::RootedObject obj(cx, args.thisv().toObjectOrNull());
    js_proxy_t *proxy = jsb_get_js_proxy(cx, obj);
    cocos2d::extension::AssetsManagerEx* cobj = (cocos2d::extension::AssetsManagerEx *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough : Invalid Native Object");
    if (argc == 1) {
        long long arg0 = 0;
        ok &= jsval_to_long_long(cx, args.get(0), &arg0);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough : Error processing arguments");
        bool ret = cobj->checkFreeMemoryIsEnough(arg0);
        JS::RootedValue jsret(cx);
        jsret = JS::BooleanValue(ret);
        JSB_PRECONDITION2(ok, cx, false, "js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough : error parsing return value");
        args.rval().set(jsret);
        return true;
    }

    JS_ReportErrorUTF8(cx, "js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough : wrong number of arguments: %d, was expecting %d", argc, 1);
    return false;
}
...
void js_register_cocos2dx_extension_AssetsManagerEx(JSContext *cx, JS::HandleObject global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("checkFreeMemoryIsEnough", js_cocos2dx_extension_AssetsManagerEx_checkFreeMemoryIsEnough, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
