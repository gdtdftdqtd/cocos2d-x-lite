...
bool seval_to_std_vector_cocos_Vec2(const se::Value& v, std::vector<cocos2d::Vec2>* ret)
{
    assert(ret != nullptr);
    assert(v.isObject());
    se::Object* obj = v.toObject();
...
namespace {
    ...
    template<typename T>
    bool std_vector_T_Vec2_to_seval(const std::vector<T>& v, se::Value* ret)
    {
        assert(ret != nullptr);
        se::HandleObject obj(se::Object::createArrayObject(v.size()));
        bool ok = true;
...
bool std_vector_Vec2_to_seval(std::vector<cocos2d::Vec2>& v, se::Value* ret)
{
    return std_vector_T_Vec2_to_seval(v, ret);
}
